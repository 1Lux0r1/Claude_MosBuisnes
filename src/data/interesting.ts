/* ---------------------------------------------------------------------------
   Блок «Возможно интересно» на Главной (п.4 ТЗ).

   Три типа карточек:
   1. Прогнозная модель налогообложения — заглушка, кнопка «Рассчитать».
   2. Персональные предложения сервисов — с полем tags (под будущую фильтрацию
      по анкете и активностям предпринимателя). В макете показаны как будто уже
      отфильтрованные; реальной фильтрации нет.
   3. Управление ликвидностью — заглушка, статус блока предварительный.

   Тексты — заглушки под реальный контент. Кнопки — нерабочие.
--------------------------------------------------------------------------- */

import type { IconName } from "../icons";

/** Персональное предложение (тип 2) — подбирается по тегам профиля/активностей. */
export interface PersonalOffer {
  id: string;
  title: string;
  /** Короткая метка для квадратной плитки на Главной */
  short: string;
  desc: string;
  icon: IconName;
  tint: string;
  action: string;
  /** Теги анкеты и активностей предпринимателя — под будущую фильтрацию. */
  tags: string[];
}

/** Карточка-заглушка (типы 1 и 3). */
export interface FeatureCard {
  title: string;
  /** Короткая метка для квадратной плитки на Главной */
  short: string;
  desc: string;
  icon: IconName;
  tint: string;
  action: string;
  /** Статус блока предварительный — показываем бейдж «Скоро». */
  preliminary?: boolean;
}

export const TAX_FORECAST_CARD: FeatureCard = {
  title: "Прогнозная модель налогообложения",
  short: "Прогноз налогообложения",
  desc: "Расчёт специальной ставки при наступлении условия — например, при трудоустройстве сотрудника с инвалидностью.",
  icon: "chart",
  tint: "#e6efff",
  action: "Рассчитать",
};

export const MOEX_TRENDS_CARD: FeatureCard = {
  title: "Тренды Московской Биржи",
  short: "Тренды МосБиржи",
  desc: "Курсы валют, ключевые индексы и настроения рынка для планирования.",
  icon: "trend-up",
  tint: "#e6efff",
  action: "Смотреть",
};

export const LIQUIDITY_CARD: FeatureCard = {
  title: "Управление ликвидностью",
  short: "Ликвидность",
  desc: "Инструменты для планирования денежного потока и размещения свободных средств.",
  icon: "coins",
  tint: "#e6f7f7",
  action: "Подробнее",
  preliminary: true,
};

export const PERSONAL_OFFERS: PersonalOffer[] = [
  { id: "po1", title: "Курс: продажи на маркетплейсах", short: "Место на маркетплейсе", desc: "Подобрано по вашей активности в логистике", icon: "grid", tint: "#ece9ff", action: "Смотреть", tags: ["маркетплейсы", "логистика"] },
  { id: "po2", title: "Субсидия для общепита", short: "Субсидия", desc: "Похоже на профиль вашей деятельности", icon: "coins", tint: "#e3f6ec", action: "Проверить", tags: ["общепит", "субсидии"] },
  { id: "po3", title: "Юрист по договорам аренды", short: "Юрист", desc: "Часто требуется бизнесу вашего профиля", icon: "shield", tint: "#fff3d4", action: "Подобрать", tags: ["аренда", "юруслуги"] },
];
